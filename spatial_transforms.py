...
class ToTensor(object):
    ...
    def __call__(self, pic):
        ...
        if pic.mode == 'I':
            ...
            # 将不可写的缓冲区复制为一个新的可写缓冲区
            buffer = bytearray(pic.tobytes())
            img = torch.frombuffer(buffer, dtype=torch.uint8)
...
class Scale(object):
    ...
    def __call__(self, img):
        """
        Args:
            img (PIL.Image or None): Image to be scaled.
        Returns:
            PIL.Image or None: Rescaled image, or None if input was None.
        """
        if img is None:  # 检查 img 是否为 None
            return None
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, img):
        ...
        """
        self.randomize_parameters()  # 每次调用时初始化 p
        if self.p < 0.5:
            return img.transpose(Image.FLIP_LEFT_RIGHT)
        return img

    def randomize_parameters(self):
        self.p = random.random()  # 随机生成 [0, 1) 范围的值
...
class RandomRotate(object):
    """Randomly rotate the given PIL.Image by a random angle within a specified range."""

    def __init__(self, max_angle=5):
        self.max_angle = max_angle  # 设置旋转角度范围
        self.interpolation = Image.BILINEAR
        self.rotate_angle = 0  # 默认值，避免未定义的情况

    def __call__(self, img):
        self.randomize_parameters()  # 初始化 rotate_angle
        ret_img = img.rotate(self.rotate_angle, resample=self.interpolation)
        return ret_img

    def randomize_parameters(self):
        # 在 (-max_angle, max_angle) 范围内生成随机角度
        self.rotate_angle = random.randint(-self.max_angle, self.max_angle)
...
class SaltImage(object):
    def __init__(self, ratio=100):
        self.ratio = ratio
        self.p = 1.0  # 初始化 p，避免未定义的情况

    def __call__(self, img):
        self.randomize_parameters()  # 确保每次调用时初始化 p
        is_PIL = isinstance(img, Image.Image)
        if is_PIL:
            img = np.asarray(img)
    ...
    def randomize_parameters(self):
        self.p = random.random()  # 生成随机的 p 值
        self.ratio = random.randint(80, 120)  # 生成随机的 ratio 值


class Dropout(object):
    """Randomly set pixels to zero in the given PIL.Image or numpy array."""

    def __init__(self, ratio=100):
        self.ratio = ratio
        self.p = 1.0  # 默认值，防止未定义 p 的情况

    def __call__(self, img):
        self.randomize_parameters()  # 初始化 p 和 ratio
        is_PIL = isinstance(img, Image.Image)
        if is_PIL:
            img = np.asarray(img)
    ...
    def randomize_parameters(self):
        self.p = random.random()  # 生成随机的 p 值
        self.ratio = random.randint(30, 50)  # 在调用时生成随机 ratio 值
