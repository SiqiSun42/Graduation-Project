...
def pil_loader(path):
    ...
    try:
        ...
        print(f"Warning: Cannot identify image file {path}. Skipping this file.")
        return None
    ...
    def __getitem__(self, index):
        if self.subset == 'validation':
            ...
            if len(clip) == 0:  # 如果所有图像都加载失败，则跳过这个 index
                print(f"Skipping corrupted testclip at index {index}")
                return None, None

            # 检查clip的长度是否满足要求
            if len(clip) < self.expected_length:
                print(f"incomplete testclip at index {index} with length {len(clip)}")
                clip += [clip[-1]] * ( self.expected_length - len(clip))

            clip = torch.stack(clip, 0).permute(1, 0, 2, 3)
            #print(f"testclip：{len(clip)}")
